...
URL\_DECODE函数的encoding设置目前只支持GBK格式。
...
-   （.）、（-）、（\*）和（\_）保持不变。
-   空格转为（+）。
-   其余字符根据指定的encoding转为字节值，encoding设置目前只支持GBK格式。然后将每个字节值表示为%xy的格式，xy是该字符值的十六进制表示方式。
...
**说明：** URL\_ENCODE函数的encoding设置目前只支持GBK格式。
