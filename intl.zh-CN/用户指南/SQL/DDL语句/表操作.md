...
-   CLUSTERED BY指定Hash Key。MaxCompute将对指定列进行Hash运算，按照Hash值分散到各个Bucket里面。
    ...
    -   INTO number\_of\_buckets BUCKETS指定了哈希桶的数目。这个数字必须提供，但应该由数据量大小来决定。此外，缺省条件下MaxCompute只能支持最多1111个reducer，所以，此处最多也只支持1111个哈希桶。您可以使用`set odps.sql.reducer.instances=xxx`来提升这个限制，但最大不得超过4000，否则会影响性能。
...
由上可见，除生命周期属性外，sale\_detail\_like的其他属性（字段类型，分区类型等）均与sale\_detail完全一致。查看表信息的更多详情请参见[表操作](intl.zh-CN/用户指南/常用命令/表操作.md#)。

如果您查看表sale\_detail\_ctas1的信息，会发现sale\_date、region两个字段仅会作为普通列存在，而不是表的分区。

您也可以使用`DESC EXTENDED table_name;`查看Hash Clustering Table的Clustering 属性，如下所示，Clustering属性将显示在Extended Info中。
