...
MaxCompute以天为单位出账单，您可以通过单击顶部导航栏中的**费用**，进入[消费记录\>消费明细](https://billing.console.aliyun.com/#/expense/outline/)查看每天的账单信息。
...
![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/11938/15561739531126_zh-CN.png)

**说明：** 包年包月中的**后付费**是指项目开通包年包月计算计费模式后，还产生的存储、下载对应的费用（存储、下载费用只有后付费）。

点击**详情**查看每个项目的消费情况，您还可以通过**使用记录**查看消费是如何产生的。

![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/11938/15561739538898_zh-CN.png)

## 使用记录 {#section_cw1_m2d_5db .section}

通过[消费记录\>消费明细](https://billing.console.aliyun.com/#/expense/outline/)，看不到这些费用是如何产生的，例如每天的存储花费、计算费用是由哪些SQL产生的、这些SQL作业是由谁提交的等信息，如果您想了解这些信息，需要通过[消费记录\>使用记录](https://billing.console.aliyun.com/#/usage/record)来下载每天的使用详细信息，如下图所示：

![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/11938/15561739531127_zh-CN.png)
...
如果您想知道**如何计算某个SQL任务的费用**，可以在下载使用记录后，打开CSV表格。查看**数据分类**中的**ComputationSql** SQL计费项。某项目SQL任务产生的费用计算公式为`计算输入数据量 * SQL复杂度 * SQL价格`。详情参见[ZH-CN\_TP\_11936.md\#section\_cx5\_hvc\_5db](intl.zh-CN/产品定价/计算计费项（按CU预付费）.md#section_cx5_hvc_5db)。

例如下图中计量信息编号20171106100629865g4iplf9的SQL任务，产生的费用是：SQL读取量\(7352600872Byte/1024/1024/1024\)\*SQL复杂度 1 \* 0.3元/GB/复杂度=2元

![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/11938/15561739539592_zh-CN.png)

如果您使用了[外部表功能](../../../../intl.zh-CN/用户指南/外部表/外部表概述.md#)，例如OTS、OSS外表，则上述表格中还会出现SQL读取量\_访问OTS（Byte）或SQL读取量\_访问OSS（Byte）信息。外表的计费公式为
...
-   首先换算成计算时`25940(core*s)=7.2055(core*h)`，`7.2055(core*h)×0.46(￥)=3.314￥`结果即为该MR作业花费的金额，所有作业的总和对应账单中MR作业计算应付金额。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/11938/15561739531132_zh-CN.png)
...
![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/11938/15561739538899_zh-CN.png)
...
如果您使用DataWorks进行[MaxCompute SQL](../../../../intl.zh-CN/使用指南/数据开发/节点类型/ODPS SQL节点.md#)开发，您可以使用图形界面上的费用预估功能估算可能产生的SQL费用。

![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/11938/155617395334166_zh-CN.png)

您也可以使用[COST SQL](../../../../intl.zh-CN/用户指南/常用命令/其他操作.md#section_xm2_sgf_vdb)命令直接在odpscmd客户端上预估SQL费用，或者调用[SQLCostTask SDK接口](http://www.javadoc.io/doc/com.aliyun.odps/odps-sdk-core/0.29.10-public)进行预估。
...
-   **分析存储收取1分钱的原因**
    ...
    1.  下载使用记录后，打开CSV表格。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/11938/15561739548900_zh-CN.png)

    2.  查看**数据分类**中的**Storage**存储计费项，发现在maxcompute\_doc项目下存储了508字节数据。

        按照[存储计费规则](intl.zh-CN/产品定价/计算计费项（按CU预付费）.md#section_qw5_hvc_5db)，存储\(508Byte/1024/1024/1024\)\*0.0192元/GB<0.01元，小于等于512M数据最低收取1分钱。
...
-   **分析数据上传和下载是否产生了费用**
    ...
    1.  点击**消费明细**详情，查看上行、下载有无收费。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/11938/15561739548901_zh-CN.png)

    2.  通过导出CSV使用记录，分析下载消耗明细，数据分类为**DownloadEx**代表公网下载计费项。

        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/11938/15561739548903_zh-CN.png)

    3.  公网下行流量产生了一条约0.036GB（38199736byte）的下行流量，根据[计量计费](intl.zh-CN/产品定价/计算计费项（按CU预付费）.md#table_xx5_hvc_5db)标准，（38199736byte/1024/1024/1024）\*0.8 元/GB=0.028元。
    4.  下载优化举例

        查看您的Tunnel设置的Service，是否因为设置为了公共网络产生费用，可参考[访问域名和数据中心](../../../../intl.zh-CN/准备工作/配置Endpoint.md#)。
...
-   **分析“不足一天”的数据存储费用**
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/11938/15561739548904_zh-CN.png)
    ...
    **说明：** 天计量范围以结束时间为准，所以最后一条数据不包括在4月4日账单中。
    ...
    按照[存储定价规则](intl.zh-CN/产品定价/计算计费项（按CU预付费）.md#table_sw5_hvc_5db)先计算24小时存储价格：
